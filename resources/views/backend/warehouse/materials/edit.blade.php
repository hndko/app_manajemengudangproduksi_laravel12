...
@section('subheader', 'Ubah data material: ' . $material->name)

@section('content')
<div class="max-w-2xl">
    <form action="{{ route('materials.update', $material) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <!-- Image Upload -->
        <div class="card" x-data="{ imagePreview: '{{ $material->image ? asset('storage/' . $material->image) : '' }}' }">
            <h3 class="text-sm font-semibold text-white mb-4 flex items-center gap-2">
                <svg class="w-4 h-4 text-primary-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex items-start gap-4">
                ...
                <div class="flex-1">
                    ...
                    @if($material->image)
                        <p class="text-xs text-neutral-400 mt-1">Gambar saat ini: {{ basename($material->image) }}</p>
                    @endif
                    @error('image')
                        <p class="text-xs text-danger-400 mt-1">{{ $message }}</p>
                    @enderror
