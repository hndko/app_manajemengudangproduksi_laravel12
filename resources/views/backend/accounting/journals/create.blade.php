...
@section('header', 'Buat Jurnal Baru')
@section('subheader', 'Catat transaksi ke jurnal umum')

@section('content')
<div class="max-w-4xl mx-auto">
    <form action="{{ route('journals.store') }}" method="POST" x-data="journalForm()" class="space-y-4">
        @csrf
        ...
        <div class="card">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <div class="relative">
                        <input type="date" id="date" name="date" value="{{ old('date', now()->format('Y-m-d')) }}" class="form-input pl-9 @error('date') border-danger-500 @enderror" required>
                ...
                <div>
                    <label class="form-label">Periode Fiskal</label>
                    <div class="form-input bg-dark-card/50 text-neutral-400">
                        {{ $fiscalPeriod ? $fiscalPeriod->name : 'Tidak ada periode aktif' }}
                    </div>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="relative">
                        <textarea id="description" name="description" rows="2" class="form-input pl-9 @error('description') border-danger-500 @enderror" placeholder="Deskripsi transaksi" required>{{ old('description') }}</textarea>
                ...
                <table class="w-full">
                    ...
                    <tbody>
                        <template x-for="(row, index) in rows" :key="index">
                            <tr class="border-b border-dark-border">
                                <td class="py-2 px-2">
                                    <select :name="`details[${index}][account_id]`" class="form-select text-sm" required>
                                ...
                                <td class="py-2 px-2">
                                    <input type="text" :name="`details[${index}][description]`" class="form-input text-sm" placeholder="Keterangan">
                                </td>
                                <td class="py-2 px-2">
